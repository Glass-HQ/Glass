...
If you have a language server installed, the diagnostics pane can show you all errors across your project. You can open it by clicking on the icon in the dock header, or running the `diagnostics: Deploy` command (`cmd-shift-m` on macOS, `ctrl-shift-m` on Windows/Linux, or `:clist` in Vim mode).
