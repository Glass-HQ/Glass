...
pub fn cache_nix_dependencies_namespace() -> Step<Use> {
    named::uses("actions", "cache", "v4")
        .add_with(("path", "/nix"))
        .add_with(("key", "nix-${{ runner.os }}-${{ hashFiles('flake.lock') }}"))
}

pub fn cache_nix_store_macos() -> Step<Use> {
    named::uses("actions", "cache", "v4")
        .add_with(("path", "~/nix-cache"))
        .add_with(("key", "nix-store-macos-${{ hashFiles('flake.lock') }}"))
}
...
pub(crate) const DEFAULT_REPOSITORY_OWNER_GUARD: &str = "(github.repository_owner == 'Glass-HQ')";
